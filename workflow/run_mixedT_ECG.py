...
from ECGMOD.multiT_preprocess import multiT_data_loader
...
def main():
    ...
    config_json = json.load(open(csv))
    jump = config_json["jump"] 

    random.seed(args.randomseed)  # Initialize the random seed 
    np.random.seed(args.randomseed) 
    print("\nSet random seed :", args.randomseed )    
 
    train_x, train_y, test_x, test_y, numy, scaleY, scalerX = multiT_data_loader(csv,NormalizeY=True,NormalizeX=True)
    train_ind = [i for i in range(len(train_x))]
    test_ind = [j for j in range(len(test_x))]
    
    sampler = OTFSampler(index=train_ind) 
    err_list, r2_list,  train_size, indlist  =  list(), list(), list(), list()
    err_unscaled_1000K ,r2_unscaled_1000K = list(), list() 
    err_unscaled_650K ,r2_unscaled_650K = list(), list()
    err_unscaled_300K ,r2_unscaled_300K = list(), list()
    err_unscaled_50K ,r2_unscaled_50K = list(), list()
    ...
    for trial in tqdm(range(1, Ntrials+1)):
        ...
        for key, val in jump.items():
            ...
            rmse, r2, _ = InferGPModel(likelihood, model, test_x[val[0]:val[1]], test_y[val[0]:val[1]], scaleY)

            print("\nValidating on {}  test set and rescaling to normal units!\n".format(key) )  
            print('\nIteration:{}, RMSE: {}, r2: {}, Test size: {}\n'.format(trial, rmse, r2, test_x[val[0]:val[1]].shape[0]))

            if key == "1000K":
                err_unscaled_1000K.append(rmse) ; r2_unscaled_1000K.append(r2) 
            elif key == "650K":
                err_unscaled_650K.append(rmse) ; r2_unscaled_650K.append(r2)
            elif key == "300K":
                err_unscaled_300K.append(rmse) ; r2_unscaled_300K.append(r2)
            elif key == "50K":
                err_unscaled_50K.append(rmse) ; r2_unscaled_50K.append(r2)

        train_size.append(x_train.shape[0] )

    _ = plot_metric_write(train_size, err_list, r2_list, err_unscaled_1000K ,r2_unscaled_1000K ,indlist, "1000K")
    _ = plot_metric_write(train_size, err_list, r2_list, err_unscaled_650K ,r2_unscaled_650K ,indlist, "650K")
    _ = plot_metric_write(train_size, err_list, r2_list, err_unscaled_300K ,r2_unscaled_300K ,indlist, "300K")
    _ = plot_metric_write(train_size, err_list, r2_list, err_unscaled_50K ,r2_unscaled_50K ,indlist, "50K")
